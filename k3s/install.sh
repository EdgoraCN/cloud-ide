...
setup_tmp() {
    ...
    TMP_GAP=${TMP_DIR}/k3s-airgap-images-amd64.tar
    cleanup() {
        code=$?
        set +e
        trap - EXIT
...
download_hash() {
    ...
    HASH_URL=http://aima.hyitec.com/sha256sum-amd64.txt
...
download_binary() {
    ...
    BIN_URL=http://aima.hyitec.com/k3s
...
download_gap() {
    #BIN_URL=${GITHUB_URL}/download/${VERSION_K3S}/k3s${SUFFIX}
    GAP_URL=http://aima.hyitec.com/k3s-airgap-images-amd64.tar
    info "Downloading binary ${GAP_URL}"
    curl -o ${TMP_GAP} -sfL ${GAP_URL} || fatal "GAP download failed"
}
setup_airgap() {
    if [ ! -f $TMP_GAP ]; then
	download_gap
    ...
    $SUDO cp $TMP_GAP /var/lib/rancher/k3s/agent/images/
}
setup_image_mirror() {
    #setup_airgap
    CT_CFG_FILE=/var/lib/rancher/k3s/agent/etc/containerd/config.toml
    while $SUDO test ! -f  $CT_CFG_FILE
    do
	echo "Waiting config file setup..."
	sleep 3
    done
    echo "try to add image mirror"
    $SUDO tee -a  ${CT_CFG_FILE} >/dev/null << EOF
  [plugins.cri.registry.mirrors]
  [plugins.cri.registry.mirrors."docker.io"]
    endpoint = ["https://dockerhub.akscn.io","https://fq8upf2e.mirror.aliyuncs.com","https://registry-1.docker.io"]
  [plugins.cri.registry.mirrors."k8s.gcr.io"]
    endpoint = ["https://gcr.azk8s.cn/google_containers","https://gcr.akscn.io/google_containers"]
  [plugins.cri.registry.mirrors."gcr.io"]
    endpoint = ["https://gcr.azk8s.cn/google_containers","https://gcr.akscn.io/google_containers"]
EOF
    $SUDO cp $CT_CFG_FILE  $CT_CFG_FILE.tmpl
}
